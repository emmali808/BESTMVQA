...
def tfidf_loading(use_tfidf, w_emb, args):
    if use_tfidf:
        ...
        if args.use_data:
            ...
            if os.path.isfile(os.path.join(args.data_dir, 'embed_tfidf_weights_cr.pkl')) == True:
                print("Loading embedding tfidf and weights from file")
                with open(os.path.join(args.data_dir ,'embed_tfidf_weights_cr.pkl'), 'rb') as f:
                    # print("####", args.data_dir)
                    w_emb = torch.load(f)
                    # import pickle
                    # w_emb = pickle.load(f)
                ...
                with open(os.path.join(args.data_dir ,'embed_tfidf_weights_cr.pkl'), 'wb') as f:
                    torch.save(w_emb, f)
