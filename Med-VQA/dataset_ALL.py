...
def _create_entry(dataroot, img, data, answer, organ_list, question_type_list):
    if None != answer:
        if ('SLAKE' in dataroot) or ('PATH' in dataroot)or ('OVQA' in dataroot) or ('Med-2019' in dataroot):
            answer.pop('img_name')
            answer.pop('qid')
        elif ('RAD' in dataroot) or ('FREE' in dataroot):
    ...
    if 'OVQA' in dataroot:
        ...
        # print(data['qid'])
        return entry
    elif ('RAD' in dataroot) or ('SLAKE' in dataroot) or ('FREE' in dataroot):
        organ2idx = {organ: idx for idx, organ in enumerate(organ_list)}
        question_type2idx = {q_t: idx for idx, q_t in enumerate(question_type_list)}
        entry = {
            'qid': data['qid'],
        ...
        # print(data['qid'])
        return entry
    elif ('PATH'in dataroot) or ('Med-2019'in dataroot) : 
        question_type2idx = {q_t: idx for idx, q_t in enumerate(question_type_list)}
        entry = {
            'qid': data['qid'],
            'image_name': data['image_name'],
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args,dictionary, dataroot='data_RAD', question_len=12):
        ...
        if 'OVQA' in dataroot:#745 322
            ...
            self.organ_list =None
            self.question_type_list = ['ABNORMALITY', 'MODALITY', 'PLANE', 'BINARY', 'ORGAN']
        ...
        # 需要用mmbert的虚拟环境读取
        # print("########", (self.label2close))
        # print("########", (self.label2open))
    ...
    def __getitem__(self, index):
        ...
        elif ('PATH' in self.args.dataset) or ('Med-2019' in self.args.dataset) :
            image_organ=None
        ...
        if ('RAD' in self.args.dataset) or ('SLAKE' in self.args.dataset) or ('OVQA' in self.args.dataset) or ('FREE' in self.args.dataset) or ('Med-2019' in self.args.dataset):
        ...
        if None!=answer:
            if ('SLAKE' in self.args.dataset) or ('PATH' in self.args.dataset) or ('Med-2019' in self.args.dataset):
                # # alter by hxj 删掉
                # if 'OVQA' in self.args.dataset:
                labels = answer['labels']
                scores = answer['scores']
                composed_target = torch.zeros(self.num_ans_candidates)  # close + open
                # 删掉
                ####### 1102 781 321
                # print("#######", self.num_ans_candidates, self.num_open_candidates, self.num_close_candidates)
                if answer_target == 0:
                    ...
                    if labels is not None:
                        ...
                        # 删掉
                        # print(labels, scores, self.num_close_candidates)
                        # exit()
                ...
                if labels is not None:
                    try:
                        composed_target.scatter_(0, labels, scores)
                    except:
                        print('a_t=0 {0},{1}'.format(qid,labels))
            elif ('RAD' in self.args.dataset)or ('FREE' in self.args.dataset):
                labels = answer['labels']
                scores = answer['scores']
                composed_target = torch.zeros(self.num_ans_candidates)  # close + open
                if answer_target == 0:
