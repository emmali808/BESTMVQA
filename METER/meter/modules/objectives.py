...
def compute_vqa(pl_module, batch, test=False):
    ...
    vqa_targets = torch.zeros(len(vqa_logits), pl_module.hparams.config["vqav2_label_size"], device=pl_module.device)
    ...
    # alter by hxj
    # print("###########", dir(pl_module))
    # print("########", len(pl_module.train_dataloader()), pl_module.global_step)
    if phase == "train" and pl_module.global_step == 1:
        # print("#######total_idx", pl_module.hparams.config['batch_size'], pl_module.current_epoch+1, int(vqa_loss), pl_module.hparams.config['record_id'])
        conn=connect()
        cursor=conn.cursor()
        sql="INSERT INTO detail (batch_size, epoch,loss,record_id) values(%s, %s, %s, %s)" %  (pl_module.hparams.config['batch_size'], pl_module.current_epoch+1, int(vqa_loss), pl_module.hparams.config['record_id'])
        cursor.execute(sql)
        conn.commit()
        cursor.close()
        conn.close()  
...
def compute_vqa_v01(pl_module, batch):
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    vqa_logits = pl_module.vqa_classifier(infer["cls_feats"])

    ## alter by hxj
    ## pl_module.hparams.config["vqav2_label_size"] 为人为设定的
    ...
    vqa_labels = batch["vqa_labels"]
    vqa_scores = batch["vqa_scores"]
    # print("#####", vqa_targets.shape, len(vqa_labels))
