...
from ..gadgets.my_metrics import Accuracy, VQAScore, Scalar, VQARADScore
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            # alter by hxj
            if phase == "test":
                # print("#######",getattr(pl_module, f"{phase}_{loss_name}_score").get_best_close_score(),
                # getattr(pl_module, f"{phase}_{loss_name}_score").get_best_open_score(),
                # getattr(pl_module, f"{phase}_{loss_name}_score").get_best_score())
                temp_a = getattr(pl_module, f"{phase}_{loss_name}_score").get_best_close_score()
                temp_b = getattr(pl_module, f"{phase}_{loss_name}_score").get_best_open_score()
                temp_c = getattr(pl_module, f"{phase}_{loss_name}_score").get_best_score()
                conn=connect()
                cursor=conn.cursor()
                sql="UPDATE `record` SET closed=%f,open=%f,`all`=%f,`status`='%s' where id=%s" % (temp_a, temp_b, temp_c,'complete', pl_module.hparams.config['record_id'])
                cursor.execute(sql)
                conn.commit()
                cursor.close()
                conn.close()   
