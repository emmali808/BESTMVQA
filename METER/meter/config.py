...
def config():
    ...
    batch_size = 2  # this is a desired batch size; pl trainer will accumulate gradients when per step batch is smaller.
...
def task_mlm_itm_clip_bert():
    ...
    val_transform_keys = ["clip"]
    learning_rate = 1e-5
    # alter by hxj  val_check_interval = 0.1
    val_check_interval = 1.0
    lr_mult_head = 5
...
@ex.named_config
def task_vqa_ovqa():
    exp_name = "task_vqa_ovqa"
    vqav2_label_size = 635
    val_check_interval = 0.5
    # max_epoch = 1

@ex.named_config
def task_vqa_rad():
    exp_name = "task_vqa_rad"
    vqav2_label_size = 4040

@ex.named_config
def task_vqa_path():
    exp_name = "task_vqa_path"
    vqav2_label_size = 4794

@ex.named_config
def task_vqa_medvqa_2019():
    exp_name = "task_vqa_medvqa_2019"
    vqav2_label_size = 79
    # batch_size = 16
...
def task_finetune_vqa_clip_bert():
    ...
    draw_false_image = 0
    learning_rate = 5e-6
    # later by hxj val_check_interval:int或 float 指定每个多少step后进行一次验证,可以是整数(表示步数)或小数(表示比例)。不能设置为1
