...
def tfidf_loading(use_tfidf, w_emb, args):
    if use_tfidf:
        ...
        if args.use_VQA:
            if os.path.isfile(os.path.join(args.VQA_dir, 'embed_tfidf_weights_mmq.pkl')) == True:
                print("Loading embedding tfidf and weights from file")
                with open(os.path.join(args.VQA_dir ,'embed_tfidf_weights_mmq.pkl'), 'rb') as f:
                    w_emb = torch.load(f)
                ...
                if 'SLAKE' in args.VQA_dir:
                    tfidf, weights = dataset_VQA.tfidf_from_questions(['train'], args, dict)
                else:
                ...
                with open(os.path.join(args.VQA_dir ,'embed_tfidf_weights_mmq.pkl'), 'wb') as f:
                    torch.save(w_emb, f)
