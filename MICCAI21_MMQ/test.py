...
def get_result_RAD(model, dataloader, device, args):
    ...
    with torch.no_grad():
        for v, q, a, ans_type, q_types, p_type in iter(dataloader):
            if 'OVQA' in args.VQA_dir:
                pass
            elif 'RAD' in args.VQA_dir and p_type[0] != "freeform":
                continue
            if args.maml:
...
if __name__ == '__main__':
    ...
    def process(args, model, eval_loader):
        ...
        if args.use_VQA:
            if 'RAD' in args.VQA_dir or 'SLAKE' in args.VQA_dir  or 'Med-2019' in args.VQA_dir:
