...
def get_result(model, dataloader, device, args):
    ...
    if 'RAD' in args.RAD_dir:
        ...
        quesntion_types = ['have', 'there', 'sectioned', 'can', 'acute', 'one', 'microscopy', 'where', 'd', 'when', 'these', 
                            'how', 'had', 'what', 'does', 'whose', 'two', 'has', 'did', 'infiltration', 'a', 'under', 'dose', 
                            'reticulin', 'were', 'are', 'is', 'was', 'the', 'do', 'by', 'impaired', 'who', 'metastatic', 'b', 'why'] 
        all_key = 'all'
    ...
    with torch.no_grad():
        for v, q, a, modal_label, ans_type, q_types, p_type in iter(dataloader):
            ...
            result[all_key]['count'] += 1.0
            result[all_key]['true'] += float(batch_score)
            result[all_key]['real'] += float(a.sum())
            ...
            for i in q_types:
                ...
                question_types_result[all_key][i]['count'] += 1.0
                question_types_result[all_key][i]['true'] += float(batch_score)
                question_types_result[all_key][i]['real'] += float(a.sum())
    ...
    def process(args, model, eval_loader):
        ...
        if args.use_RAD:
            ...
            if 'RAD' in args.RAD_dir  or 'SLAKE' in args.RAD_dir  or 'Med-2019' in args.RAD_dir:
                ...
                # 'other', 'yes/no', 'all'
                a=np.round((result['yes/no']['true']+result['number']['true'])/(result['yes/no']['count']+result['yes/no']['count']))
                b=result['other']['score_percent']
                c=result['all']['score_percent']            
            conn = connect()
