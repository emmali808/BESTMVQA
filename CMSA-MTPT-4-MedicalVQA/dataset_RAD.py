...
def _create_entry(img, data, answer):
    ...
    entry = {
        ...
        'image_organ': data['image_organ'] if 'image_organ' in data.keys() else 'unknow',
        }
    return entry
...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    if 'SLAKE' in dataroot or 'Med-2019' in dataroot or 'PATH' in dataroot or 'OVQA' in dataroot:
        for ans_js in answers:
            ans_js['image_name'] = ans_js.pop('img_name')
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        # self.organ2label = {'ABD': 0, 'HEAD': 1, 'CHEST': 2, 'unknow': 0}
        #
        if 'OVQA' in args.RAD_dir:
            self.organ2label = {'HAND': 0, 'LEG': 1, 'CHEST': 2, 'HEAD': 3}
            args.n_classes = 3
        elif 'RAD' in args.RAD_dir:
            self.organ2label = {'ABD': 0, 'HEAD': 1, 'CHEST': 2}
            args.n_classes = 3
        elif 'PATH' in args.RAD_dir:
            self.organ2label = {'unknow': 0}
            args.n_classes = 3
        elif 'SLAKE' in args.RAD_dir:
            self.organ2label = {'Brain_Face': 0, 'Chest_mediastinal': 1, 'Chest_heart': 2, 'Chest_lung': 3, 
                                'Abdomen': 4, 'Brain': 5, 'Brain_Tissue': 6, 'Pelvic Cavity': 7, 'Neck': 8, 'Lung':9}
            args.n_classes = 10
