        ...
        if sample is not None:
            try:
                with torch.no_grad() if eval else contextlib.ExitStack():
                    modal_label = sample[3]
                    answers = sample[2]
                    img_data = sample[0][1]
                    # MEVF loss computation
                    if self.args.autoencoder:
                        features, decoder = self.model(sample[0], sample[1])
                    elif self.args.distmodal:
                        features, modal = self.model(sample[0], sample[1])
                    else:
                    ...
                    if self.args.distmodal:
                        loss_modal = torch.nn.CrossEntropyLoss(reduction='sum')(modal, modal_label)
                        if self.args.dwa:
                            if self.cnt == 0:
                                self.loss_prev2 = np.array([loss.item(), loss_modal.item()])
                                self.cnt += 1
                            elif self.cnt == 1:
                                self.loss_prev = np.array([loss.item(), loss_modal.item()])
                                self.cnt += 1
                            else:
                                w = self.loss_prev / (self.loss_prev2 + 1e-8)
                                w = w / self.T
                                self.weights = self.softmax(w) * 2
                            loss = self.weights[0]*loss + self.weights[1]*loss_modal
                            if self.cnt > 1:
                                self.loss_prev2 = self.loss_prev.copy()
                                self.loss_prev = np.array([loss.item(), loss_modal.item()])
                        else:
                            loss = loss + (loss_modal * self.args.modal_alpha)
...
def compute_score_with_logits(logits, labels):
    ...
    # print("#####", labels.size(), logits.size()) # torch.Size([32, 4122]) torch.Size([32])
    # print("#####", *labels.size(), logits.device)
