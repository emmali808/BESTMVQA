...
answer_types = ['plane', 'modality', 'organ', 'abnormality', 'binary']
...
def parse_args():
    ...
    parser.add_argument('--medvqa2019_dir', type=str,
                        help='medvqa2019_dir')
...
def get_result(model, dataloader, device, args):
    ...
    with torch.no_grad():
        for v, q, a, ans_type, q_types in iter(dataloader):
            ...
            # print((a.data.sum()))
            # print("#####", final_preds.softmax(1), a)
            # exit()
            ascore = sentence_bleu(final_preds.softmax(1), a.data.softmax(1), weights = [1])
            print(ascore)
            # batch_score_bleu = calculate_bleu_score(final_preds,a.data).sum()
    ...
    for i in answer_types:
        ...
        true_count=true_count+result[i]['true']
    accu=round((true_count/all_count)*100, 1)
    print(accu)
    conn=connect()
    sql="UPDATE record SET all=%s, where record_id=%s" % (accu,args.record_id)
    cursor = conn.cursor()
    cursor.execute(sql)
    conn.commit()
    return result
...
if __name__ == '__main__':
    ...
    if args.use_RAD:
        ...
        eval_dset = dataset_Medvqa.VQAFeatureDataset(args.split, args, dictionary)

    batch_size = args.batch_size

    constructor = 'build_%s' % args.model
    model = getattr(base_model_Medvqa, constructor)(eval_dset, args)
    # print(model)
    eval_loader = DataLoader(eval_dset, batch_size, shuffle=False, num_workers=0, pin_memory=True, collate_fn=utils_medvqa.trim_collate)
    ...
    def process(args, model, eval_loader):
        ...
        if args.use_RAD:
            ...
            result = get_result(model, eval_loader, args.device, args)
