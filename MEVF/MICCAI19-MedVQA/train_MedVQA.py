...
if __name__ == '__main__':
    ...
    #         val_dset = dataset_RAD.VQAFeatureDataset('val', args, dictionary, question_len=args.question_len)
    ...
    eval_loader = DataLoader(valdataset, batch_size = args.batch_size, shuffle=False, num_workers = args.num_workers)
    test_loader = DataLoader(testdataset, batch_size = args.batch_size, shuffle=False, num_workers = args.num_workers)
        

    batch_size = args.batch_size
    # Create VQA model
    constructor = 'build_%s' % args.model
    model = getattr(base_model, constructor)(traindataset, args)
    optim = None
    epoch = 0
    # load snapshot
    ...
    train_loader = DataLoader(traindataset, batch_size, shuffle=True, num_workers=0, collate_fn=utils.trim_collate, pin_memory=True)
    eval_loader = DataLoader(valdataset, batch_size, shuffle=False, num_workers=0, collate_fn=utils.trim_collate, pin_memory=True)
