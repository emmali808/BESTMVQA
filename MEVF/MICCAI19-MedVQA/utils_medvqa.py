...
def tfidf_loading(use_tfidf, w_emb, args):
    if use_tfidf:
        if args.use_RAD:
            dict = dataset_Medvqa.Dictionary.load_from_file(os.path.join(args.medvqa2019_dir, 'dictionary.pkl'))

        # load extracted tfidf and weights from file for saving loading time
        if args.use_RAD:
            if os.path.isfile(os.path.join(args.medvqa2019_dir, 'embed_tfidf_weights.pkl')) == True:
                print("Loading embedding tfidf and weights from file")
                with open(os.path.join(args.medvqa2019_dir ,'embed_tfidf_weights.pkl'), 'rb') as f:
                    w_emb = torch.load(f)
                ...
                tfidf, weights = dataset_Medvqa.tfidf_from_questions(['train'], args, dict)
                w_emb.init_embedding(os.path.join(args.medvqa2019_dir, 'glove6b_init_300d.npy'), tfidf, weights)
                with open(os.path.join(args.medvqa2019_dir ,'embed_tfidf_weights.pkl'), 'wb') as f:
                    torch.save(w_emb, f)
