...
def _load_dataset(dataroot, name, img_id2val, label2ans,args):
    ...
    for sample, answer in zip(samples, answers):
        ...
        ## alter by hxj
        # print(sample, answer)
        # if "SLAKE" in dataroot or "PATH" in dataroot:
        if "SLAKE" in dataroot or "RAD" in dataroot:
        # if "SLAKE" in dataroot in dataroot:
            answer['image_name'] = answer.pop('img_name')
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        # print("#####", ans2label_path, len(self.ans2label))
        ...
        self.entries = _load_dataset(dataroot, name, self.img_id2idx, self.label2ans,self.args)
        # load image data for MAML module
        if args.maml:
            # TODO: load images
            if 'RAD' not in args.RAD_dir and 'SLAKE' not in args.RAD_dir and 'PATH' not in args.RAD_dir:
    ...
    def tensorize(self):
        if self.args.maml:
            if 'RAD' in self.args.RAD_dir or 'SLAKE' in self.args.RAD_dir or 'PATH' in self.args.RAD_dir:
    ...
    def __getitem__(self, index):
        ...
        if None!=answer:
            ...
            if labels is not None:
                ...
                # if answer_type.upper() =='OPEN' or answer_type.upper() =='CLOSED':
                #     print("#####################", labels, scores, self.num_ans_candidates)
