...
def tfidf_loading(use_tfidf, w_emb, args):
    if use_tfidf:
        if args.use_RAD:
            dict = dataset_ALL.Dictionary.load_from_file(os.path.join(args.RAD_dir, 'dictionary.pkl'))

        # print("######", dict.word2idx, dict.idx2word)
        ...
        if args.use_RAD:
            if os.path.isfile(os.path.join(args.RAD_dir, 'embed_tfidf_weights.pkl')) == True:
                ...
                with open(os.path.join(args.RAD_dir ,'embed_tfidf_weights.pkl'), 'rb') as f:
                    w_emb = torch.load(f,map_location=torch.device('cpu'))
                ...
                tfidf, weights = dataset_ALL.tfidf_from_questions(['train'], args, dict)
                # print(args.RAD_dir)
