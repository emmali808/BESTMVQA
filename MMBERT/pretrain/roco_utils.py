...
def load_mlm_data(args):
    #train_path = os.path.join(args.data_dir,'trainval','radiology')
    train_path = os.path.join(args.data_dir,'trainval')
    #val_path = os.path.join(args.data_dir,'trainval','radiology')
    val_path = os.path.join(args.data_dir,'trainval')
    #test_path = os.path.join(args.data_dir,'trainval','radiology')
    test_path = os.path.join(args.data_dir,'trainval')

    train_image_names = os.listdir(os.path.join(train_path,'radiology','images'))
    val_image_names = os.listdir(os.path.join(val_path,'radiology','images'))
    test_image_names = os.listdir(os.path.join(test_path,'radiology','images'))

    #replace the param error_bad_lines with on_bad_lines -ljr 
    #train_data = pd.read_csv(os.path.join(train_path,'radiologytraindata_test.csv'),error_bad_lines=False,header=0)
    train_data = pd.read_csv(os.path.join(train_path,'radiologytraindata_test.csv'),on_bad_lines='skip',header=0)
    train_data = train_data[(train_data['name']+".jpg").isin(train_image_names)]

    #val_data = pd.read_csv(os.path.join(val_path, 'radiologyvaldata_test.csv'),error_bad_lines=False,header=0)
    val_data = pd.read_csv(os.path.join(val_path, 'radiologyvaldata_test.csv'),on_bad_lines='skip',header=0)
    val_data = val_data[(val_data['name']+".jpg").isin(val_image_names)]

    # test_data = pd.read_csv(os.path.join(test_path, 'testdata.csv'))
    # test_data = test_data[(test_data['name']+".jpg").isin(test_image_names)]
...
def train_one_epoch(loader, model, criterion, optimizer, scaler, device, args, epoch):
    ...
    #PREDS = np.concatenate(PREDS).cpu().numpy()
    TARGETS = torch.cat(TARGETS).cpu().numpy()
    #TARGETS = np.concatenate(TARGETS).cpu().numpy()
...
class ROCO(Dataset):
    ...
    def __getitem__(self, idx):
        ## alter by hxj
        # print(idx, self.df[:5])
        name = self.df[idx,0]+'.jpg' 

        # path = os.path.join(self.path, self.mode, 'radiology', 'images',name)
        # print("####", name)
        path = os.path.join(self.path, 'trainval', 'radiology', 'images',name)
        ...
        ## 是不是有问题？？？？
        caption = self.df[idx, 1].strip()      ##new
        # caption = self.df[idx, 2].strip()   --old
