...
if __name__ == '__main__':
    ...
    print(test_df)
    robot_test_df=pd.DataFrame(data=[[args.image_path,args.question,2,"test","modality","OPEN"]],columns=['img_id','question','answer','mode','category','answer_type'])
    print(robot_test_df)
    ...
    robottestdataset = VQAMed(robot_test_df, imgsize = args.image_size, tfm = test_tfm, args = args)
    ...
    robottestloader = DataLoader(robottestdataset, batch_size = args.batch_size, shuffle=False, num_workers = args.num_workers)
    ...
    test_loss, predictions, acc, bleu = test(robottestloader, model, criterion, device, scaler, args, robot_test_df,idx2ans)
    print("final:")
    print(idx2ans[predictions[0]]) #answer