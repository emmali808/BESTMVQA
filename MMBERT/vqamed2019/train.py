...
if __name__ == '__main__':
    ...
    ## 测试代码 删掉
    # train_df = train_df[:round(len(train_df)*0.01)]
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, _, accu, _, _ = train_one_epoch(trainloader, model, optimizer, criterion, device, scaler, args, idx2ans,train_df)
    ...
    test_loss, predictions, acc, bleu = test(testloader, model, criterion, device, scaler, args, test_df,idx2ans)
    print("final:")
    sql="UPDATE `record` SET closed=%f,open=%f,`all`=%f,`status`='%s' where id=%s" % (acc['closed_acc'],acc['open_acc'],acc['total_acc'],'complete',args.record_id)
    cursor.execute(sql)
    conn.commit()
    print(acc)
    print("idx2ans[predictions[1]]")
    print(idx2ans[predictions[1]])