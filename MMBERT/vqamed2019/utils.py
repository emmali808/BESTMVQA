...
def load_data(args, remove = None):
    ...
    traindf = pd.read_csv(os.path.join(args.data_dir, 'ImageClef-2019-VQA-Med-Training', 'traindf.csv'))
    valdf = pd.read_csv(os.path.join(args.data_dir, 'ImageClef-2019-VQA-Med-Validation', 'valdf.csv'))
    testdf = pd.read_csv(os.path.join(args.data_dir, 'ImageClef-2019-VQA-Med-Test', 'testdf.csv'))
    ...
    testdf['img_id'] = testdf['img_id'].apply(lambda x: os.path.join(args.data_dir, 'ImageClef-2019-VQA-Med-Test', 'test_images', x + '.jpg'))
    ...
    # 删掉
    # print("#####valdf", valdf['answer'])

    valdf['answer'] = valdf['answer'].str.lower()

    # 删掉
    # print("#####testdf", testdf['answer'])

    testdf['answer'] = testdf['answer'].str.lower()

    # szx modified
    traindf['answer_type']= traindf['answer'].apply(lambda x:'CLOSED' if ('yes' in x or 'no' in x) else 'OPEN')
    valdf['answer_type']= valdf['answer'].apply(lambda x:'CLOSED' if ('yes' in x or 'no' in x) else 'OPEN')
    testdf['answer_type']= testdf['answer'].apply(lambda x:'CLOSED' if ('yes' in x or 'no' in x) else 'OPEN')

    traindf = traindf.sample(frac = args.train_pct)
    print(traindf['answer'],traindf['answer_type'])
...
def train_one_epoch(loader, model, optimizer, criterion, device, scaler, args, idx2ans,train_df):
...
def test(loader, model, criterion, device, scaler, args, val_df,idx2ans):
    ...
    if args.category:
        ...
        acc = {'total_acc': np.round(total_acc, 1), 'closed_acc': np.round(closed_acc, 1), 'open_acc': np.round(open_acc, 1)}
