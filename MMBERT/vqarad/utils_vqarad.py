...
class VQAMed(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "PATH" in self.args.data_dir:
            path = self.df[idx,1]
            question = self.df[idx, 5]
        ...
        img = Image.open(path)
        ## later by hxj
        if img.mode == 'CMYK' or img.mode == 'L'or img.mode == 'RGBA':
            img = img.convert('RGB')

        # print("#########", path, np.array(img).shape, img.mode)
...
def calculate_bleu_score(preds,targets, idx2ans):
    ...
    #     print("#####", idx2ans[target], idx2ans[pred])
...
def train_one_epoch(loader, model, optimizer, criterion, device, scaler, args, val_df, idx2ans):
    ...
    if "PATH" in args.data_dir:
        closed_acc = (PREDS[val_df['answer_type']=='yes/no'] == TARGETS[val_df['answer_type']=='yes/no']).mean() * 100.
        open_acc = (PREDS[val_df['answer_type']!='yes/no'] == TARGETS[val_df['answer_type']!='yes/no']).mean() * 100.        
    else:
...
def validate(loader, model, criterion, device, scaler, args, val_df, idx2ans):
    ...
    if "PATH" in args.data_dir:
        closed_bleu = calculate_bleu_score(PREDS[val_df['answer_type']=='yes/no'],TARGETS[val_df['answer_type']=='yes/no'],idx2ans)
        open_bleu = calculate_bleu_score(PREDS[val_df['answer_type']!='yes/no'],TARGETS[val_df['answer_type']!='yes/no'],idx2ans)
    else:
