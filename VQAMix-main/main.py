...
if __name__ == '__main__':
    ...
    if ('RAD' in args.RAD_dir) or ('Med-2019' in args.RAD_dir) or ('SLAKE' in args.RAD_dir):
        args.feat_dim = 64
    else:
        args.feat_dim = 32

    # Load dictionary and RAD training dataset
    # if ('RAD' not in args.RAD_dir) and ('SLAKE' not in args.RAD_dir) and ('Med-2019' not in args.RAD_dir):
    if ('RAD' not in args.RAD_dir) and ('SLAKE' not in args.RAD_dir):
        ...
        if 'Med-2019' in args.RAD_dir:
            eval_dset = dataset_VQA.VQAFeatureDataset('test', args, dictionary, question_len=args.question_len)
        else:
