...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str,  default='model_epoch_99.pth', help='the model_name')
...
def get_result_pathVQA(model, dataloader, device, args):
    ...
    print(result['yes/no']['score'])
    print(result['other']['score'])
    print(result['all']['score'])
    a=result['yes/no']['score']*100
    b=result['other']['score']*100
    c=result['all']['score']*100
    conn = connect()
    cursor = conn.cursor()
    sql="UPDATE `record` SET closed=%f,open=%f,`all`=%f,`status`='%s' where id=%s" % (a,b,c,'complete',args.record_id)
    cursor.execute(sql)
    conn.commit()
    cursor.close()
    conn.close()
    ...
    print('VQA RAD Overall ACC:', all_batch_score * 100 / len(dataloader))
...
def get_result(model, dataloader, device, args):
    ...
    if "OVQA" in args.RAD_dir:
        quesntion_types = ['Attribute Other', 'Abnormality', 'Modality', 'Organ System', 'PLANE', 'Condition' ]
        answer_types = ['CLOSED', 'OPEN', 'ALL']    
    elif 'RAD' in args.RAD_dir:
        answer_types = ['CLOSED', 'OPEN', 'ALL']
        quesntion_types = ['COUNT', 'COLOR', 'ORGAN', 'PRES', 'PLANE', 'MODALITY', 'POS', 'ABN', 'SIZE', 'OTHER', 'ATTRIB']
    elif 'SLAKE' in args.RAD_dir:
        answer_types = ['CLOSED', 'OPEN', 'ALL']
        quesntion_types = ['Plane', 'Size', 'KG', 'Position', 'Organ', 'Color', 'Shape', 'Quantity', 'Modality', 'Abnormality']   
    elif 'Med-2019' in args.RAD_dir:
        answer_types = ['CLOSED', 'OPEN', 'ALL']
        quesntion_types = ['BINARY', 'PLANE', 'MODALITY', 'ORGAN', 'ABNORMALITY']   
    ...
    a=result['CLOSED']['score_percent']
    b=result['OPEN']['score_percent']
    c=result['ALL']['score_percent']
    print(result)
    conn = connect()
...
if __name__ == '__main__':
    ...
    if 'RAD' in args.RAD_dir or 'SLAKE' in args.RAD_dir  or 'Med-2019' in args.RAD_dir:
        args.feat_dim = 64
    else:
        args.feat_dim = 32
    ...
    def process(args, model, eval_loader):
        # if os.path.exists(os.path.join(args.input, 'model_epoch_best.pth')):
        #     model_path = os.path.join(args.input, 'model_epoch_best.pth')
        if os.path.exists(os.path.join(args.input, args.model_name)):
